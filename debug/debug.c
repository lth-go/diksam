...
void DBG_assert_func(char *fmt, ...) {
    ...
    assert_func(st_current_controller->debug_write_fp, st_current_file_name,
                st_current_line, st_assert_expression, fmt, ap);
...
void DBG_panic_func(char *fmt, ...) {
    ...
    va_start(ap, fmt);
    initialize_debug_write_fp();
    panic_func(st_current_controller->debug_write_fp, st_current_file_name,
...
void DBG_set_expression(char *expression) { st_assert_expression = expression; }
