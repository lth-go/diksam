...
typedef struct { int label_address; } LabelTable;
...
static DVM_LocalVariable *copy_local_variables(FunctionDefinition *fd,
                                               int param_count) {
    int i;
    ...
    for (i = 0, derive = src->derive; derive; derive = derive->next, i++) {
        switch (derive->tag) {
            case DVM_FUNCTION_DERIVE:
                ...
                dest->derive[i].u.function_d.parameter = copy_parameter_list(
                    derive->u.function_d.parameter_list, &param_count);
...
static void generate_boolean_expression(DVM_Executable *cf, Expression *expr,
                                        OpcodeBuf *ob) {
...
static void generate_int_expression(DVM_Executable *cf, Expression *expr,
                                    OpcodeBuf *ob) {
    DVM_ConstantPool cp;
    int cp_idx;

    if (expr->u.int_value >= 0 && expr->u.int_value < 256) {
        generate_code(ob, expr->line_number, DVM_PUSH_INT_1BYTE,
                      expr->u.int_value);
    } else if (expr->u.int_value >= 0 && expr->u.int_value < 65536) {
        generate_code(ob, expr->line_number, DVM_PUSH_INT_2BYTE,
                      expr->u.int_value);
    } else {
...
static void generate_double_expression(DVM_Executable *cf, Expression *expr,
                                       OpcodeBuf *ob) {
    DVM_ConstantPool cp;
    int cp_idx;
...
static void generate_string_expression(DVM_Executable *cf, Expression *expr,
                                       OpcodeBuf *ob) {
    DVM_ConstantPool cp;
    int cp_idx;
...
static int get_opcode_type_offset(DVM_BasicType basic_type) {
    switch (basic_type) {
...
static void generate_identifier_expression(DVM_Executable *exe, Block *block,
                                           Expression *expr, OpcodeBuf *ob) {
    if (expr->u.identifier.is_function) {
        generate_code(ob, expr->line_number, DVM_PUSH_FUNCTION,
        ...
        generate_code(
            ...
            DVM_PUSH_STACK_INT +
                get_opcode_type_offset(
                    expr->u.identifier.u.declaration->type->basic_type),
...
static void generate_pop_to_identifier(Declaration *decl, int line_number,
                                       OpcodeBuf *ob) {
    if (decl->is_local) {
        generate_code(
            ob, line_number,
...
static void generate_assign_expression(DVM_Executable *exe, Block *block,
                                       Expression *expr, OpcodeBuf *ob,
    ...
    generate_pop_to_identifier(
        expr->u.assign_expression.left->u.identifier.u.declaration,
        expr->line_number, ob);
}

static void generate_binary_expression(DVM_Executable *exe, Block *block,
...
static void generate_logical_and_expression(DVM_Executable *exe, Block *block,
                                            Expression *expr, OpcodeBuf *ob) {
    int false_label;
...
static void generate_logical_or_expression(DVM_Executable *exe, Block *block,
                                           Expression *expr, OpcodeBuf *ob) {
    int true_label;
...
static void generate_cast_expression(DVM_Executable *exe, Block *block,
                                     Expression *expr, OpcodeBuf *ob) {
...
static void generate_inc_dec_expression(DVM_Executable *exe, Block *block,
    ...
    generate_pop_to_identifier(
        expr->u.inc_dec.operand->u.identifier.u.declaration, expr->line_number,
        ob);
}
...
static void generate_expression(DVM_Executable *exe, Block *current_block,
    ...
    switch (expr->kind) {
        ...
        case ASSIGN_EXPRESSION:
            ...
            break;
        case ADD_EXPRESSION:
            generate_binary_expression(exe, current_block, expr, DVM_ADD_INT,
                                       ob);
            break;
        case SUB_EXPRESSION:
            generate_binary_expression(exe, current_block, expr, DVM_SUB_INT,
                                       ob);
            break;
        case MUL_EXPRESSION:
            generate_binary_expression(exe, current_block, expr, DVM_MUL_INT,
                                       ob);
            break;
        case DIV_EXPRESSION:
            generate_binary_expression(exe, current_block, expr, DVM_DIV_INT,
                                       ob);
            break;
        case MOD_EXPRESSION:
            generate_binary_expression(exe, current_block, expr, DVM_MOD_INT,
                                       ob);
            break;
        case EQ_EXPRESSION:
            generate_binary_expression(exe, current_block, expr, DVM_EQ_INT,
                                       ob);
            break;
        case NE_EXPRESSION:
            generate_binary_expression(exe, current_block, expr, DVM_NE_INT,
                                       ob);
            break;
        case GT_EXPRESSION:
            generate_binary_expression(exe, current_block, expr, DVM_GT_INT,
                                       ob);
            break;
        case GE_EXPRESSION:
            generate_binary_expression(exe, current_block, expr, DVM_GE_INT,
                                       ob);
            break;
        case LT_EXPRESSION:
            generate_binary_expression(exe, current_block, expr, DVM_LT_INT,
                                       ob);
            break;
        case LE_EXPRESSION:
            generate_binary_expression(exe, current_block, expr, DVM_LE_INT,
                                       ob);
            break;
        ...
        case MINUS_EXPRESSION:
            generate_expression(exe, current_block, expr->u.minus_expression,
                                ob);
            generate_code(
                ob, expr->line_number,
...
static void generate_expression_statement(DVM_Executable *exe, Block *block,
                                          Expression *expr, OpcodeBuf *ob) {
    if (expr->kind == ASSIGN_EXPRESSION) {
        ...
        generate_inc_dec_expression(exe, block, expr, expr->kind, ob, DVM_TRUE);
    } else {
...
static void generate_if_statement(DVM_Executable *exe, Block *block,
    ...
    for (elsif = if_s->elsif_list; elsif; elsif = elsif->next) {
        ...
        generate_code(ob, statement->line_number, DVM_JUMP_IF_FALSE,
                      if_false_label);
        generate_statement_list(exe, elsif->block, elsif->block->statement_list,
                                ob);
        generate_code(ob, statement->line_number, DVM_JUMP, end_label);
        set_label(ob, if_false_label);
    }
...
static void generate_while_statement(DVM_Executable *exe, Block *block,
                                     Statement *statement, OpcodeBuf *ob) {
    int loop_label;
    ...
    generate_statement_list(exe, while_s->block, while_s->block->statement_list,
                            ob);
...
static void generate_for_statement(DVM_Executable *exe, Block *block,
                                   Statement *statement, OpcodeBuf *ob) {
    int loop_label;
    ...
    generate_statement_list(exe, for_s->block, for_s->block->statement_list,
                            ob);
...
static void generate_return_statement(DVM_Executable *exe, Block *block,
                                      Statement *statement, OpcodeBuf *ob) {
...
static void generate_break_statement(DVM_Executable *exe, Block *block,
                                     Statement *statement, OpcodeBuf *ob) {
    ...
    for (block_p = block; block_p; block_p = block_p->outer_block) {
        ...
        if (block_p->type == WHILE_STATEMENT_BLOCK) {
            ...
            if (!strcmp(break_s->label,
                        block_p->parent.statement.statement->u.while_s.label)) {
                break;
            }
            ...
            if (!strcmp(break_s->label,
                        block_p->parent.statement.statement->u.for_s.label)) {
                break;
            }
        }
    }
    if (block_p == NULL) {
        dkc_compile_error(statement->line_number, LABEL_NOT_FOUND_ERR,
                          STRING_MESSAGE_ARGUMENT, "label", break_s->label,
                          MESSAGE_ARGUMENT_END);
    }

    generate_code(ob, statement->line_number, DVM_JUMP,
                  block_p->parent.statement.break_label);
}

static void generate_continue_statement(DVM_Executable *exe, Block *block,
                                        Statement *statement, OpcodeBuf *ob) {
...
static void generate_initializer(DVM_Executable *exe, Block *block,
                                 Statement *statement, OpcodeBuf *ob) {
        ...
        switch (pos->statement->type) {
            case EXPRESSION_STATEMENT:
                generate_expression_statement(
                    exe, current_block, pos->statement->u.expression_s, ob);
                break;
            case IF_STATEMENT:
                generate_if_statement(exe, current_block, pos->statement, ob);
                break;
            case WHILE_STATEMENT:
                generate_while_statement(exe, current_block, pos->statement,
                                         ob);
                break;
            case FOR_STATEMENT:
            ...
            case RETURN_STATEMENT:
                generate_return_statement(exe, current_block, pos->statement,
                                          ob);
                break;
            case BREAK_STATEMENT:
                generate_break_statement(exe, current_block, pos->statement,
                                         ob);
                break;
            case CONTINUE_STATEMENT:
                generate_continue_statement(exe, current_block, pos->statement,
                                            ob);
                break;
            case TRY_STATEMENT:
                break;
...
static void fix_labels(OpcodeBuf *ob) {
    ...
    for (i = 0; i < ob->size; i++) {
        if (ob->code[i] == DVM_JUMP || ob->code[i] == DVM_JUMP_IF_TRUE ||
...
static void copy_function(FunctionDefinition *src, DVM_Function *dest) {
    ...
    dest->parameter =
        copy_parameter_list(src->parameter, &dest->parameter_count);
    if (src->block) {
...
static void add_functions(DKC_Compiler *compiler, DVM_Executable *exe) {
    ...
    for (fd = compiler->function_list, i = 0; fd; fd = fd->next, i++) {
        ...
        if (fd->block) {
            ...
            exe->function[i].need_stack_size = calc_need_stack_size(
                exe->function[i].code, exe->function[i].code_size);
        } else {
