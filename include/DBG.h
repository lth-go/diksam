...
#define DBG_assert(expression, arg)                                         \
    ((expression) ? (void)(0)                                               \
                  : ((DBG_set(DBG_CURRENT_CONTROLLER, __FILE__, __LINE__)), \
                     (DBG_set_expression(#expression)), DBG_assert_func arg))
#define DBG_panic(arg) \
    ((DBG_set(DBG_CURRENT_CONTROLLER, __FILE__, __LINE__)), DBG_panic_func arg)
#define DBG_debug_write(arg)                                \
    ((DBG_set(DBG_CURRENT_CONTROLLER, __FILE__, __LINE__)), \
     DBG_debug_write_func arg)
#endif /* DBG_NO_DEBUG */

typedef enum { DBG_TRUE = 1, DBG_FALSE = 0 } DBG_Boolean;
