...
static void add_functions(DVM_VirtualMachine *dvm, DVM_Executable *executable) {
    ...
    for (src_idx = 0, dest_idx = dvm->function_count;
        ...
        if (!executable->function[src_idx].is_implemented) continue;
        dvm->function[dest_idx].name =
...
static void convert_code(DVM_VirtualMachine *dvm, DVM_Executable *exe,
    ...
    for (i = 0; i < code_size; i++) {
        if (code[i] == DVM_PUSH_STACK_INT || code[i] == DVM_PUSH_STACK_DOUBLE ||
            code[i] == DVM_PUSH_STACK_STRING || code[i] == DVM_POP_STACK_INT ||
...
void DVM_add_executable(DVM_VirtualMachine *dvm, DVM_Executable *executable) {
    ...
    convert_code(dvm, executable, executable->code, executable->code_size,
                 NULL);
...
static void expand_stack(DVM_VirtualMachine *dvm, int need_stack_size) {
    ...
    if (rest <= need_stack_size) {
        ...
        dvm->stack.stack = MEM_realloc(
            dvm->stack.stack, dvm->stack.alloc_size * sizeof(DVM_Value));
        dvm->stack.pointer_flags =
...
static void invoke_diksam_function(DVM_VirtualMachine *dvm, Function **caller_p,
    ...
    *exe_p = callee->u.diksam_f.executable;
    ...
    expand_stack(
        dvm,
        CALL_INFO_ALIGN_SIZE + callee_p->local_variable_count +
            (*exe_p)->function[callee->u.diksam_f.index].need_stack_size);
...
static void return_function(DVM_VirtualMachine *dvm, Function **func_p,
    ...
    callInfo = (CallInfo *)&dvm->stack
                   .stack[*sp_p - 1 - callee_p->local_variable_count -
...
#define STI_WRITE(dvm, sp, r)                                             \
    ((dvm)->stack.stack[(dvm)->stack.stack_pointer + (sp)].int_value = r, \
     (dvm)->stack.pointer_flags[(dvm)->stack.stack_pointer + (sp)] =      \
         DVM_FALSE)
            ...
            case DVM_INVOKE: {
                ...
                if (dvm->function[func_idx].kind == NATIVE_FUNCTION) {
                    ...
                    invoke_diksam_function(
                        dvm, &func, &dvm->function[func_idx], &code, &code_size,
                        &pc, &dvm->stack.stack_pointer, &base, &exe);
                }
                break;
            }
            case DVM_RETURN:
