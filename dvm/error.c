...
static void format_message(DVM_Executable *exe, Function *func, int pc,
                           ErrorDefinition *format, VString *v, va_list ap) {
    int i;
...
static void self_check() {
    ...
    if (strcmp(dvm_error_message_format[RUNTIME_ERROR_COUNT_PLUS_1].format,
               "dummy") != 0) {
        DBG_panic(
            ("runtime error message format error. "
             "RUNTIME_ERROR_COUNT_PLUS_1..%d\n",
             RUNTIME_ERROR_COUNT_PLUS_1));
    }
}

static int conv_pc_to_line_number(DVM_Executable *exe, Function *func, int pc) {
...
void dvm_error(DVM_Executable *exe, Function *func, int pc, RuntimeError id,
    ...
    format_message(exe, func, pc, &dvm_error_message_format[id], &message, ap);
